...
if ( ! function_exists( 'alt_elementor_pro_render_mini_cart_item' ) ) {
	function alt_elementor_pro_render_mini_cart_item( $cart_item_key, $cart_item ) {
		...
		<div class="elementor-menu-cart__product woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
			...
			<div class="elementor-menu-cart__product-image product-thumbnail alt-test">
				<?php
...
<?php } else { ?>
	<div class="elementor-menu-cart__products woocommerce-mini-cart cart woocommerce-cart-form__contents">
		...
		foreach ( $cart_items as $cart_item_key => $cart_item ) {
			alt_elementor_pro_render_mini_cart_item( $cart_item_key, $cart_item );
		}

		// $cart_gift_items = apply_filters('alt_get_mini_cart_gift', WC()->cart->get_cart() );
		// if( ! empty($cart_gift_items) ) {
		// 	$text_for_minicart = get_field('text_for_minicart', 'option');
		// 	echo '<li class="alt-minicart-item-label"><span>' . $text_for_minicart .'</span></li>';
		// 	foreach ( $cart_gift_items as $cart_item_key => $cart_item ) {
		// 		alt_elementor_pro_render_mini_cart_item( $cart_item_key, $cart_item );
		// 	}
		// }
